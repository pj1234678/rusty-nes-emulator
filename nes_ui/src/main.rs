...
impl WavWriter {
    fn finalize(&mut self) -> std::io::Result<()> {
        ...
        let sample_rate = nes_core::AUDIO_SAMPLE_RATE as u32;
...
fn get_controller_state(
    ...
    // Controllers: 1 controller → P2, 2+ controllers → P1+P2
    // Keyboard is always P1 regardless of controllers
...
fn main() {
    ...
    let rom_path = match rom_path {
        Some(p) => p,
        None => {
            eprintln!("Usage: nes_rs <rom> [--cpu-log] [--audio-output <file>]");
            std::process::exit(1);
        }
    };

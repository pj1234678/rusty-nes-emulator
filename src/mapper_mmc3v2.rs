...
impl MapperMmc3v2 {
    ...
    pub fn new(cart: Cartridge) -> MapperMmc3v2 {
        let mut mapper = MapperMmc3v2 {
            cart,
            ram: [0; 8192],
            vram: [0; 2048],
    ...
    fn update_banks(&mut self) {
        ...
        for i in 0..4 {
            let bank = prg_banks[i] as i32 + (self.outer_bank as i32 * 4);
            self.offset_prg[i] = get_bank_offset(prg_len, 8 * 1024, bank);
        }

...
impl MapperRambo1 {
    ...
    pub fn new(cart: Cartridge) -> MapperRambo1 {
        ...
        let num_prg = mapper.prg_len / 0x2000;
        mapper.prg_bank[0] = 0;
        ...
        match addr {
            0x0000..=0x1FFF => {
                ...
                let idx = if self.chr_power_of_two {
                    (bank * 0x400 + offset) & self.chr_mask
                } else {
                    (bank * 0x400 + offset) % self.chr_len
                };
            ...
            0x8000..=0xFFFF => {
                ...
                let idx = if self.prg_power_of_two {
                    (bank * 0x2000 + offset) & self.prg_mask
                } else {
                    (bank * 0x2000 + offset) % self.prg_len
                };
        ...
        match addr {
            0x0000..=0x1FFF => {
                if self.chr_len > 0 {
                    ...
                    let bank = self.chr_bank[bank_idx] as usize;
                    let offset = (addr & 0x3FF) as usize;
                    let loc = if self.chr_power_of_two {
                        (bank * 0x400 + offset) & self.chr_mask
                    } else {
                        (bank * 0x400 + offset) % self.chr_len
                    };
        ...
        match addr {
            0x8000..=0xFFFF => {
                ...
                Some(if self.prg_power_of_two {
                    ((bank * 0x2000 + offset) & self.prg_mask) as u32
                } else {
                    ((bank * 0x2000 + offset) % self.prg_len) as u32
                })
            }
            _ => None,
        }

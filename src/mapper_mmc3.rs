...
impl MapperMmc3 {
    ...
    fn write_register(&mut self, addr: u16, val: u8) {
        match addr {
            ...
            0xA000..=0xBFFF if (addr & 1 == 1) => self.reg_ram_protect = val,
    ...
    fn check_a12(&mut self, addr: u16) {
        ...
        if a12 {
            // Require A12 to have been low for at least 3 consecutive PPU reads to count as a rising edge
            if self.a12_low_reads >= 3 {
...
impl Mapper for MapperMmc3 {
    fn peek(&mut self, addr: u16) -> u8 {
        // Any PPU access dynamically updates A12 states and IRQ clocks
        if addr <= 0x3FFF {
            self.check_a12(addr);
        }
    ...
    fn write_state_to(&self, writer: &mut dyn Write) -> io::Result<()> {
        ...
        self.reg_ram_protect.write(writer)?;
        ...
        self.a12_low_reads.write(writer)
    }
    ...
    fn read_state_from(&mut self, reader: &mut dyn Read) -> io::Result<()> {
        ...
        self.reg_ram_protect = ReadState::read(reader)?;
        ...
        self.a12_low_reads = ReadState::read(reader).unwrap_or(0);
        Ok(())
    }
}
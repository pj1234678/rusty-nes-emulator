...
impl MapperQuattro {
    ...
    pub fn new(cart: Cartridge) -> MapperQuattro {
...
impl Mapper for MapperQuattro {
    ...
    fn poke(&mut self, addr: u16, val: u8) {
        match addr {
            ...
            0x8000..=0xBFFF => {
                // Low bank select (ignored in simple mode, but some carts use it)
            }
            0xC000..=0xFFFF => {
                if self.num_prg_banks > 0 {
                    self.prg_bank = ((val as usize) >> 3) & (self.num_prg_banks - 1);
                }
            }
            _ => {}
        };

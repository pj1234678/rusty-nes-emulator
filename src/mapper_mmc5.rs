...
impl MapperMmc5 {
    ...
    fn prg_bank_offset(&self, bank: u8, bank_size: usize) -> usize {
        ...
        let rom_size = self.cart.prg_rom.len();
    ...
    fn chr_bank_offset(&self, bank: u8) -> usize {
        let chr_size = self.cart.chr_rom.len();
...
if addr < 0x4000 {
        ...
        match addr {
            ...
            0x8000..=0xFFFF => {
                ...
                if is_rom {
                    let rom_len = self.cart.prg_rom.len();
                    if rom_len == 0 { return 0; }
                    self.cart.prg_rom[offset % rom_len]
                } else {
    ...
    fn poke(&mut self, addr: u16, val: u8) {
        ...
        match addr {
            0x0000..=0x1FFF => {
                if self.cart.chr_rom.is_empty() { return; }
                let offset = self.get_chr_offset(addr, false);
                if offset < self.cart.chr_rom.len() {

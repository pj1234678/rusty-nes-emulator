use std::io::{self, Read, Write};

use crate::impl_write_state_generic_array;

use super::nes::{State, AUDIO_SAMPLES_PER_FRAME};
use super::save_state::{ReadState, WriteState};

mod dmc;
mod noise;
mod pulse;
mod triangle;

const FRAME_INTERVAL: u64 = 7457; // This remains correct for the initial wait after an $4017 reset
const FRAME_INTERVALS_4: [u64; 4] = [7458, 7456, 7458, 7458];
const FRAME_INTERVALS_5: [u64; 5] = [7458, 7456, 7458, 7458, 7452];
const FULL_AUDIO_BUFFER_LEN: usize = AUDIO_SAMPLES_PER_FRAME * 40;

const LENGTH_TABLE: [u8; 32] = [
    10, 254, 20, 2, 40, 4, 80, 6, 160, 8, 60, 10, 14, 12, 26, 14, 12, 16, 24, 18, 48, 20, 96, 22,
    192, 24, 72, 26, 16, 28, 32, 30,
];

impl_write_state_generic_array!(f32, 800);

pub struct ApuState {
    pub audio_buffer: [f32; AUDIO_SAMPLES_PER_FRAME],
    full_audio_buffer: Vec<f32>,
    audio_index: usize,
    frame_cycle_counter: usize,

    last_cpu_cycle: u64,
    cpu_cycles: u64,

    sequence_counter: u64,
    next_seq_phase: usize,
    sequencer_mode: u8,
    irq_enabled: bool,
    irq_pending: bool,
    pending_write_data: u8,
    pending_write_cycles: u8,
    hpf_in: f32,
    hpf_out: f32,

    pulse1: pulse::Pulse,
    pulse2: pulse::Pulse,
    triangle: triangle::Triangle,
    noise: noise::Noise,
    dmc: dmc::Dmc,
}

impl WriteState for ApuState {
    fn write(&self, writer: &mut dyn Write) -> io::Result<()> {
        self.audio_buffer.write(writer)?;
        self.full_audio_buffer.write(writer)?;
        self.audio_index.write(writer)?;
        self.frame_cycle_counter.write(writer)?;
        self.last_cpu_cycle.write(writer)?;
        self.cpu_cycles.write(writer)?;
        self.sequence_counter.write(writer)?;
        self.next_seq_phase.write(writer)?;
        self.sequencer_mode.write(writer)?;
        self.irq_enabled.write(writer)?;
        self.irq_pending.write(writer)?;
        self.pending_write_data.write(writer)?;
        self.pending_write_cycles.write(writer)?;
        self.hpf_in.write(writer)?;
        self.hpf_out.write(writer)?;
        self.pulse1.write(writer)?;
        self.pulse2.write(writer)?;
        self.triangle.write(writer)?;
        self.noise.write(writer)?;
        self.dmc.write(writer)
    }
}

impl ReadState for ApuState {
    fn read(reader: &mut dyn Read) -> io::Result<Self> {
        Ok(ApuState {
            audio_buffer: ReadState::read(reader)?,
            full_audio_buffer: ReadState::read(reader)?,
            audio_index: ReadState::read(reader)?,
            frame_cycle_counter: ReadState::read(reader)?,
            last_cpu_cycle: ReadState::read(reader)?,
            cpu_cycles: ReadState::read(reader)?,
            sequence_counter: ReadState::read(reader)?,
            next_seq_phase: ReadState::read(reader)?,
            sequencer_mode: ReadState::read(reader)?,
            irq_enabled: ReadState::read(reader)?,
            irq_pending: ReadState::read(reader)?,
            pending_write_data: ReadState::read(reader)?,
            pending_write_cycles: ReadState::read(reader)?,
            hpf_in: ReadState::read(reader)?,
            hpf_out: ReadState::read(reader)?,
            pulse1: ReadState::read(reader)?,
            pulse2: ReadState::read(reader)?,
            triangle: ReadState::read(reader)?,
            noise: ReadState::read(reader)?,
            dmc: ReadState::read(reader)?,
        })
    }
}

impl ApuState {
    pub fn new() -> ApuState {
        ApuState {
            audio_buffer: [0.0f32; AUDIO_SAMPLES_PER_FRAME],
            full_audio_buffer: vec![0.0f32; FULL_AUDIO_BUFFER_LEN],
            audio_index: 0,
            frame_cycle_counter: 0,

            last_cpu_cycle: 0,
            cpu_cycles: 0,

            sequence_counter: FRAME_INTERVAL,
            next_seq_phase: 0,
            sequencer_mode: 0,
            irq_enabled: false,
            irq_pending: false,
            pending_write_data: 0,
            pending_write_cycles: 0,
            hpf_in: 0.0,
            hpf_out: 0.0,
            pulse1: pulse::Pulse::new_pulse1(),
            pulse2: pulse::Pulse::new_pulse2(),
            triangle: triangle::Triangle::new(),
            noise: noise::Noise::new(),
            dmc: dmc::Dmc::new(),
        }
    }

    #[inline]
    pub fn check_irq(&self) -> bool {
        self.irq_pending || self.dmc.is_irq_pending()
    }
}

pub fn complete_frame(s: &mut State) {
    catch_up(s);

    let num_samples = s.apu.audio_index;
    if num_samples == 0 {
        return;
    }

    let step = (num_samples as f32) / (AUDIO_SAMPLES_PER_FRAME as f32);

    for i in 0..AUDIO_SAMPLES_PER_FRAME {
        let idx = ((i as f32) * step) as usize;
        s.apu.audio_buffer[i] = if idx < num_samples { s.apu.full_audio_buffer[idx] } else { 0.0 };
    }
}

#[inline]
pub fn start_frame(s: &mut State) {
    s.apu.frame_cycle_counter = 0;
    s.apu.audio_index = 0;
}

#[inline]
pub fn catch_up(s: &mut State) {
    let cpu_cycles = s.cpu.cycles - s.apu.last_cpu_cycle;
    if cpu_cycles == 0 {
        return;
    }
    emulate(s, cpu_cycles);
}

macro_rules! do_common_cycle {
    ($s:expr) => {
        if $s.apu.pending_write_cycles > 0 {
            $s.apu.pending_write_cycles -= 1;
            if $s.apu.pending_write_cycles == 0 {
                $s.apu.next_seq_phase = 0;
                $s.apu.sequence_counter = FRAME_INTERVAL;
                if ($s.apu.pending_write_data & 0b1000_0000) != 0 {
                    handle_frame_quarter($s);
                    handle_frame_half($s);
                }
            }
        }
        $s.apu.sequence_counter -= 1;
        if $s.apu.sequence_counter == 0 {
            match $s.apu.next_seq_phase {
                0 => { handle_frame_quarter($s); }
                1 => { handle_frame_quarter($s); handle_frame_half($s); }
                2 => { handle_frame_quarter($s); }
                3 => {
                    handle_frame_quarter($s);
                    if $s.apu.sequencer_mode == 0 {
                        handle_frame_half($s);
                        if $s.apu.irq_enabled { $s.apu.irq_pending = true; }
                    }
                }
                4 => { handle_frame_quarter($s); handle_frame_half($s); }
                _ => unreachable!(),
            }
            $s.apu.next_seq_phase =
                ($s.apu.next_seq_phase + 1) % (4 + ($s.apu.sequencer_mode as usize));
            if $s.apu.sequencer_mode == 0 {
                $s.apu.sequence_counter = FRAME_INTERVALS_4[$s.apu.next_seq_phase];
            } else {
                $s.apu.sequence_counter = FRAME_INTERVALS_5[$s.apu.next_seq_phase];
            }
        }
        $s.apu.triangle.clock();
    };
}

macro_rules! do_sample_cycle {
    ($s:expr) => {
        $s.apu.pulse1.clock();
        $s.apu.pulse2.clock();
        $s.apu.noise.clock();
        dmc::Dmc::clock($s);
        let pulse1_out = $s.apu.pulse1.output() as f32;
        let pulse2_out = $s.apu.pulse2.output() as f32;
        let triangle_out = $s.apu.triangle.output() as f32;
        let noise_out = $s.apu.noise.output() as f32;
        let dmc_out = $s.apu.dmc.output() as f32;
        let pulse_sum = pulse1_out + pulse2_out;
        let pulse_out = if pulse_sum == 0.0 { 0.0 } else { 95.88f32 / ((8128f32 / pulse_sum) + 100f32) };
        let tnd_denom = (triangle_out / 8227f32) + (noise_out / 12241f32) + (dmc_out / 22638f32);
        let tnd_out = if tnd_denom == 0.0 { 0.0 } else { 159.79f32 / ((1f32 / tnd_denom) + 100f32) };
        let raw_sample = pulse_out + tnd_out;
        let alpha = 0.996f32;
        let filtered_sample = alpha * $s.apu.hpf_out + alpha * (raw_sample - $s.apu.hpf_in);
        $s.apu.hpf_in = raw_sample;
        $s.apu.hpf_out = filtered_sample;
        $s.apu.full_audio_buffer[$s.apu.audio_index] = filtered_sample;
        $s.apu.audio_index += 1;
    };
}

pub fn emulate(s: &mut State, cycles: u64) {
    s.apu.last_cpu_cycle = s.cpu.cycles;
    let mut remaining = cycles;

    while remaining >= 2 {
        let sample_on_first = (s.apu.cpu_cycles & 0x1) == 0;

        do_common_cycle!(s);
        s.apu.frame_cycle_counter += 1;
        s.apu.cpu_cycles += 1;
        if sample_on_first {
            do_sample_cycle!(s);
        }

        do_common_cycle!(s);
        s.apu.frame_cycle_counter += 1;
        s.apu.cpu_cycles += 1;
        if !sample_on_first {
            do_sample_cycle!(s);
        }

        remaining -= 2;
    }

    if remaining > 0 {
        do_common_cycle!(s);
        s.apu.frame_cycle_counter += 1;
        s.apu.cpu_cycles += 1;
        if s.apu.cpu_cycles & 0x1 == 1 {
            do_sample_cycle!(s);
        }
    }
}

#[inline]
fn handle_frame_quarter(s: &mut State) {
    s.apu.pulse1.clock_frame_quarter();
    s.apu.pulse2.clock_frame_quarter();
    s.apu.triangle.clock_frame_quarter();
    s.apu.noise.clock_frame_quarter();
    s.apu.dmc.clock_frame_quarter();
}

#[inline]
fn handle_frame_half(s: &mut State) {
    s.apu.pulse1.clock_frame_half();
    s.apu.pulse2.clock_frame_half();
    s.apu.triangle.clock_frame_half();
    s.apu.noise.clock_frame_half();
    s.apu.dmc.clock_frame_half();
}

pub fn peek_register(s: &mut State, register: u16) -> u8 {
    catch_up(s);
    if register == 0x4015 {
        let val = (s.apu.pulse1.is_enabled() as u8)
            | ((s.apu.pulse2.is_enabled() as u8) << 1)
            | ((s.apu.triangle.is_enabled() as u8) << 2)
            | ((s.apu.noise.is_enabled() as u8) << 3)
            | ((s.apu.dmc.is_enabled() as u8) << 4)
            | ((s.apu.irq_pending as u8) << 6)
            | ((s.apu.dmc.is_irq_pending() as u8) << 7);
        s.apu.irq_pending = false;
        val
    } else {
        0
    }
}

pub fn poke_register(s: &mut State, register: u16, data: u8) {
    catch_up(s);

    match register {
        0x4000..=0x4003 => s.apu.pulse1.poke_register(register, data),
        0x4004..=0x4007 => s.apu.pulse2.poke_register(register, data),
        0x4008..=0x400B => s.apu.triangle.poke_register(register, data),
        0x400C..=0x400F => s.apu.noise.poke_register(register, data),
        0x4010..=0x4013 => s.apu.dmc.poke_register(register, data),
        0x4015 => {
            s.apu.pulse1.set_enable_flag((data & 0b0000_0001) != 0);
            s.apu.pulse2.set_enable_flag((data & 0b0000_0010) != 0);
            s.apu.triangle.set_enable_flag((data & 0b0000_0100) != 0);
            s.apu.noise.set_enable_flag((data & 0b0000_1000) != 0);
            s.apu.dmc.set_enable_flag((data & 0b0001_0000) != 0);
            s.apu.dmc.clear_irq();
        }
        0x4017 => {
            s.apu.sequencer_mode = (data & 0b1000_0000) >> 7;
            s.apu.irq_enabled = (data & 0b0100_0000) == 0;

            if !s.apu.irq_enabled {
                s.apu.irq_pending = false;
            }

            s.apu.pending_write_data = data;
            s.apu.pending_write_cycles = if s.apu.cpu_cycles & 0x1 == 0 { 3 } else { 4 };
        }
        _ => {}
    }
}
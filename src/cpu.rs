...
impl WriteState for CpuState {
    fn write(&self, writer: &mut dyn Write) -> io::Result<()> {
        ...
        self.status_c.write(writer)?;
        self.status_z.write(writer)?;
        self.status_i.write(writer)?;
        self.status_d.write(writer)?;
        self.status_v.write(writer)?;
        self.status_n.write(writer)?;
...
impl ReadState for CpuState {
    fn read(reader: &mut dyn Read) -> io::Result<Self> {
        Ok(CpuState {
            ...
            status_c: ReadState::read(reader)?,
            status_z: ReadState::read(reader)?,
            status_i: ReadState::read(reader)?,
            status_d: ReadState::read(reader)?,
            status_v: ReadState::read(reader)?,
            status_n: ReadState::read(reader)?,
...
pub fn emulate(s: &mut State, min_cycles: u64) -> u64 {
    ...
    while s.cpu.cycles < end_cycles {
        if s.cpu.pending_interrupt != InterruptKind::None {
            handle_interrupt(s);
        } else if !s.cpu.status_i && (s.mapper.check_irq() || s.apu.check_irq()) {
        ...
        if s.debug.cpu_log {
            eprintln!(
                "[cpu] {:04X}  {:02X} A:{:02X} X:{:02X} Y:{:02X} P:{:02X} SP:{:02X} PPU:{:3},{:3} CYC:{}",
        ...
        match opcode {
            ...
            0x6B => {
                ...
                s.cpu.a = result;
                s.cpu.status_z = s.cpu.a == 0;
                s.cpu.status_n = s.cpu.a & 0x80 > 0;
                s.cpu.status_v = (s.cpu.a >> 6) ^ (s.cpu.a >> 5) & 1 == 1;
                s.cpu.status_c = (s.cpu.a >> 6) & 1 == 1;
            }

...
// DEBUG: budget for the $2002 read log below, so a long run doesn't flood stderr.
static STATUS_READ_BUDGET: AtomicI64 = AtomicI64::new(5000);
...
pub fn emulate(s: &mut State, cycles: u64) {
    ...
    while cycles_left > 0 {
        ...
        // FIX applied: Evaluate sprites and scrolling on scanline < 240 OR pre-render scanline 261
        if (s.ppu.scanline < 240 || s.ppu.scanline == 261) && rendering_enabled {
            // Only render pixels on visible scanlines
            if s.ppu.scanline < 240 && s.ppu.tick >= 1 && s.ppu.tick <= 256 {
                render_pixel(s);
            }
...
fn render_pixel(s: &mut State) {
    ...
    // --- CRT OVERSCAN CROP ---
    // Simulates a TV bezel by hiding the top and bottom 8 scanlines.
    if y < 8 || y >= 232 {
    ...
    let pixel = COLORS[(s.ppu.palette[(col & 0x1F) as usize] & 0x3F) as usize];
    let frame = &mut s.ppu.frame_buffer;
    let i = ((y * FRAME_WIDTH) + x) * FRAME_DEPTH;
...
pub fn peek_register(s: &mut State, register: u16) -> u8 {
    ...
    s.ppu.latch = match register {
        2 => {
            ...
            // DEBUG: log PPUSTATUS reads so vblank-polling loops are visible.
            let budget = STATUS_READ_BUDGET.fetch_sub(1, Ordering::Relaxed);
            if budget > 0 {
                //eprintln!(
               //     "[ppu] read $2002 -> {:02X} (vbl={}) PC={:04X} frame={} scan={:3} tick={:3}",
                //    data, s.ppu.vblank, s.cpu.pc, s.ppu.frames, s.ppu.scanline, s.ppu.tick
               // );
            } else if budget == 0 {
                eprintln!("[ppu] $2002 read log budget exhausted, no more lines will print");
            }
...
pub fn poke_register(s: &mut State, register: u16, data: u8) {
    ...
    s.ppu.latch = data;
    //eprintln!("[ppu] write reg={} val=0x{:02X} at frame={}, scanline={}, tick={}", register, data, s.ppu.frames, s.ppu.scanline, s.ppu.tick);
    match register {
        0 => {
            // PPUCTRL
            // FIX applied: Store old NMI flag to trigger retroactively if enabled mid-vblank
